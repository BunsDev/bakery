...
const StakeGDAO: React.FC<StakeProps> = ({}) => {
  const tokenName = "GDAO"
  const [requestedApproval, setRequestedApproval] = useState(false)

  const allowance = useAllowanceStaking()
  const {onApprove} = useApproveStaking()

  const tokenBalance = useTokenBalance(contractAddresses.gdao[CHAIN_ID])
      ...
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            ...
            <Label text={`GDAO Tokens Available`}/>
          </StyledCardHeader>
          <StyledCardActions>
            {!allowance.toNumber() ? (
              <Button
